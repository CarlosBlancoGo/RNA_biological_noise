---
title: "Isoseq mice data transcript models length exploration, bias correction and RBN correlation"
...
ggsave("pacbio_filtered_transcript_models_length_distribution.png", p, path = "/home/carblan/Documents/R_files/TFM/figures/Length_bias", width = 10, height = 7)


p <- ggplot(combined_df, aes(x = pool, y = length, fill = pool, color = pool)) +
  geom_violin(alpha = 0.3) +
  geom_boxplot(alpha = 0.5, width = 0.5, outlier.shape = NA) + 
  ...
  stat_compare_means( aes(label = ..p.signif..), 
                        label.x = 1.5, hide.ns = TRUE, method = "t.test", show.legend=FALSE)
...
# Save the plot
ggsave("pacbio_filtered_read_length_distribution.png", p, path = "/home/carblan/Documents/R_files/TFM/figures/Length_bias", width = 10, height = 7)

combined_df$group <- factor(combined_df$group, levels = c("young", "old"))
combined_df$pool <- factor(combined_df$pool, levels = c("pool 1", "pool 2"))

res.aov2 <- aov(read_lengths ~ group * pool, data = combined_df)
anova_results <- Anova(res.aov2, type = "III")
print(Anova(res.aov2, type = "III"))

p <- ggplot(combined_df, aes(x = pool, y = read_lengths, fill = pool, color = pool)) +
  geom_boxplot(alpha = 0.5) +
  labs(x = "Pool", y = "Read length", fill = "Pool", color = "Pool") +
  scale_fill_conesa(palette = "warm") +
  scale_color_conesa(palette = "warm") +
  theme_minimal() +
  my_theme
ggsave("pacbio_filtered_read_length_distribution_pools.png", p, path = "/home/carblan/Documents/R_files/TFM/figures/Length_bias", width = 10, height = 7)
...
ggsave("pacbio_filtered_read_length_distribution_age.png", p, path = "/home/carblan/Documents/R_files/TFM/figures/Length_bias")

agg_df <- combined_df %>%
  group_by(group, pool) %>%
  summarize(avg_length = mean(read_lengths), .groups = 'drop')

# Create interaction plot with average percentages
p <- ggplot(agg_df, aes(x = pool, y = avg_length, color = group, group = group)) +
  geom_point(size = 3) +
  geom_line(size = 1) +
  labs(
    x = "Pool",
...
ggsave("pacbio_filtered_read_length_distribution_interaction_plot.png", p, path = "/home/carblan/Documents/R_files/TFM/figures/RBNcapture/PacBio")

```


# Structural categories/all length distribution (genes weighted lengths):
